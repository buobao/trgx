...
public abstract class ActivityCmd {
    ...
    /**
     * 获取历史活动节点
     * only if the task and previous task are in the same execution can be rollBacked in this version.
     * @return
     */
    public List<HistoricActivityInstance> getProcessInstanceHistoryActivities() {
        return Context.getCommandContext().getDbSqlSession()
                .createHistoricActivityInstanceQuery().executionId(
                        execution.getId()).list();
    }
}
