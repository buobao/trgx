// 纯JS省市区三级联动（Jquery修改版）
// 2013-11-28 by qiushihua
var addressInit = function(_cmbProvince, _cmbCity, _cmbArea, defaultProvince, defaultCity, defaultArea) {
	...
	function changeCity() {
		cmbArea.empty();
		var prov_id = cmbProvince.get(0).selectedIndex;
		var city_id = cmbCity.get(0).selectedIndex;
		...
		temp_html=select_prehtml;
			$.each(provinceList[prov_id].cityList[city_id].areaList,function(i,dist){
				var selected = "";
				if(defaultArea == dist){selected = " selected = 'selected' "}
				temp_html+="<option value='"+dist+"'" + selected + ">"+dist+"</option>";
			});
			
		cmbArea.html(temp_html);
		cmbArea.selectmenu( "refresh" );
	}
	...
	function changeProvince() {
		...
		temp_html=select_prehtml;
		if(provinceList[prov_id].cityList.length > 0){
			$.each(provinceList[prov_id].cityList,function(i,city){
				var selected = "";
				if(defaultCity == city.name){selected = " selected = 'selected' "}
				temp_html+="<option value='"+city.name+"'" + selected + ">"+city.name+"</option>";
			});
		}
		cmbCity.html(temp_html);
	...
	function initProvince() {
		...
		temp_html=select_prehtml;
		$.each(provinceList,function(i,prov){
			var selected = "";
			if(defaultProvince == prov.name){selected = " selected = 'selected' "}
			temp_html+="<option value='"+prov.name+"'" + selected + ">"+prov.name+"</option>";
		});
		cmbProvince.html(temp_html);
	...
	/*setTimeout(function(){
		if(defaultProvince!=null){
			cmbProvince.val(defaultProvince);
			changeProvince();
			setTimeout(function(){
				if(defaultCity!=null){
					cmbCity.val(defaultCity);
					changeCity();
					setTimeout(function(){
						if(defaultArea!=null){
							cmbArea.val(defaultArea);
						};
					},1);
				};
			},1);
...
var provinceList = [
{name:'北京', cityList:[		   
{name:'市区', areaList:['东城区','西城区','崇文区','宣武区','朝阳区','丰台区','石景山区','海淀区','门头沟区','房山区','通州区','顺义区','昌平区','大兴区','怀柔区','平谷区']},		   
{name:'县', areaList:['密云县','延庆县']}
]},
{name:'上海', cityList:[		   
{name:'市区', areaList:['黄浦区','卢湾区','徐汇区','长宁区','静安区','普陀区','闸北区','虹口区','杨浦区','闵行区','宝山区','嘉定区','浦东新区','金山区','松江区','青浦区','南汇区','奉贤区']},		   
{name:'县', areaList:['崇明县']}
]},
{name:'天津', cityList:[		   
{name:'市区', areaList:['和平区','河东区','河西区','南开区','河北区','红桥区','塘沽区','汉沽区','大港区','东丽区','西青区','津南区','北辰区','武清区','宝坻区']},		   
{name:'县', areaList:['宁河县','静海县','蓟　县']}
]},
{name:'重庆', cityList:[		   
{name:'市区', areaList:['万州区','涪陵区','渝中区','大渡口区','江北区','沙坪坝区','九龙坡区','南岸区','北碚区','万盛区','双桥区','渝北区','巴南区','黔江区','长寿区']},		   
