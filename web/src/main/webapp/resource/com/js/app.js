...
$(document).ready(function() {
	...
	$('button[data-loading-text]').on('click', function() {
		...
		//触发按钮
		url = $('label.active input[name="activity"]',btn.parent().parent()).attr('id');
		container = $('.ajax-notifications');
		loadURL(url, container);
	...
	$('#refresh').click(function(e) {
		$.SmartMessageBox({
			title : "<i class='fa fa-refresh' style='color:green'></i> 重新设置提醒",
			content : "确定重新设置用户个性设置吗？",
			buttons : '[否][是]'
	...
	$('#logout a').click(function(e) {
		...
		// ask verification
		$.SmartMessageBox({
			title : "<i class='fa fa-sign-out txt-color-orangeDark'></i> 注销 <span class='txt-color-orangeDark'><strong>" + $('#profile').text() + "</strong></span> ?",
			content : "如无需再登陆，注销后请关闭浏览器。",
			buttons : '[取消][确定]'
	...
	function logout() {
		window.location = $("#logout a").attr("href");//$.loginURL;
	}
...
function loadURL(url, container,opt) {
	var defaults={
		data:null
	};
...
function loadMoreURL(url, container) {
	...
	$.ajax({
		...
		error : function(xhr, ajaxOptions, thrownError) {
			container.append('<h4 style="margin-top:10px; display:block; text-align:left"><i class="fa fa-warning txt-color-orangeDark"></i> 错误 404! 页面没找到！</h4>');
		},
		async : true
	});
...
function loadListview(container,vdata,callback){
	...
	if($ul.length > 0){
		page = container.attr("mp-page");
		pageKeyword = container.attr("mp-keyword");
	}
	...
	$.ajax({
		...
		success : function(data) {
			...
			//alert(pageNumber + "-" + pageCount + "-" + pageKeyword)
			...
			if(parseInt(pageNumber)>1){
				$("a[key=prePage]").parent().removeClass("disabled");
			}else{
				$("a[key=prePage]").parent().addClass("disabled");
			}
			if(parseInt(pageNumber)>=parseInt(pageCount)){
				$("a[key=nextPage]").parent().addClass("disabled");
			}else{
				$("a[key=nextPage]").parent().removeClass("disabled");
			}

			$ul.attr("mp-page",pageNumber);
			$ul.attr("mp-keyword",pageKeyword);

			$("a[key=prePage]").unbind('click').bind("click",function(e){
			...
			$("a[key=nextPage]").unbind('click').bind("click",function(e){
				//alert('d')
				var pageNumber = $ul.attr("mp-page");
				$ul.attr("mp-page",parseInt(pageNumber)+1);
				loadListview(container,{},callback);
 ...
 * @param callback
 */
function loadUrlByPage(url,container,vdata,callback){
	var page = 1;
	var pageKeyword = "";
	var $ul = container;
	...
	$.ajax({
		...
		success : function(data) {
			...
			$("a[key=prePage]").unbind('click').bind("click",function(e){
				var pageNumber = $ul.attr("mp-page");
				$ul.attr("mp-page",parseInt(pageNumber)-1);
				loadUrlByPage(url,container,{},callback);
				e.preventDefault();
				e.stopPropagation();
			})
			$("a[key=nextPage]").unbind('click').bind("click",function(e){
...
function jqGridStyle() {
	...
	$(".ui-jqgrid-labels, .ui-search-toolbar").children().removeClass(
	...
	$(".ui-icon.ui-icon-disk").removeClass().addClass("fa fa-save").parent(
		".btn-primary").removeClass("btn-primary").addClass(
		"btn-success");
	$(".ui-icon.ui-icon-cancel").removeClass().addClass("fa fa-times")
		.parent(".btn-primary").removeClass("btn-primary").addClass(
		"btn-danger");
